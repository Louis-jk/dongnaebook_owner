...
const OrderRejectCancelModal = props => {
  ...
  const rejectSendHandler = () => {
    let rejectTxt = rejectTypes.filter(reject => reject.type_id === selectedType);
    let filteredText = "";
  ...
  const cancelSendHandler = () => {
    let rejectTxt = cancelTypes.filter(reject => reject.type_id === selectedType);
    let filteredText = "";
    if (rejectTxt[0].type_id !== 6) {
      filteredText = rejectTxt[0].type_description;
    } else {
      ...
      <Modal
        ...
        <KeyboardAvoidingView
          ...
          <View
            ...
            <Text style={{...BaseStyle.ko14, ...BaseStyle.font_bold}}>{`주문 ${
              modalType === "reject" ? "거부" : "취소"
            } 사유를 선택해주세요`}</Text>
            <TouchableOpacity
              activeOpacity={1}
              onPress={toggleModal}
          ...
          {modalType === "reject" ? (
            <View style={{zIndex: -1}}>
              <TouchableOpacity
                ...
                <Text style={{...BaseStyle.ko15, ...BaseStyle.font_666}}>거부하기</Text>
              </TouchableOpacity>
            </View>
          ) : (
            <View style={{zIndex: -1}}>
